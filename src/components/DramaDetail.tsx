import { ArrowLeft, Clock, Star, Globe, Film, Play, Bookmark, Share2 } from "lucide-react";
...
export default function DramaDetail({ drama, cast, onBack, onCastClick, onWatch }: Props) {
  return (
    <div className="min-h-screen bg-[#0A0A0A]">
      {/* Header bar */}
      <div className="sticky top-0 z-40 bg-[#0A0A0A]/95 backdrop-blur border-b border-[#D4AF37]/15">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 mb-10">
          ...
          <div className="shrink-0">
            <div className={`w-36 h-52 sm:w-44 sm:h-64 rounded-md overflow-hidden border border-[#D4AF37]/20 bg-gradient-to-br ${drama.color} shadow-lg`}>
          ...
          <div className="flex-1 space-y-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="px-2 py-0.5 text-[10px] font-bold tracking-widest uppercase bg-[#C41E3A] text-white rounded-sm">
                  {drama.type}
                </span>
        ...
        {cast.length > 0 && (
          <div>
            <h2 className="text-[#D41E3A] text-[11px] font-bold tracking-[0.15em] uppercase mb-4">Cast</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {cast.map((member) => (
                ...
                >
                  {/* Avatar */}
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#C41E3A] to-[#1B2A4A] flex items-center justify-center shrink-0 border border-[#D4AF37]/15">
